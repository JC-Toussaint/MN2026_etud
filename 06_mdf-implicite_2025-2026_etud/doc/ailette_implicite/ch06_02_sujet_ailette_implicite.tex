...
\lhead{} 
\chead{} 
\rhead{\bfseries Méthodes des différences finies - Application} 
\lfoot{JCT }
%\cfoot{} 
%\rfoot{\thepage}
...
\begin{tabular}{|l|c|c|}
  \hline
Conductivité thermique $k_{th}$ & 237 & $W m^{-1} K^{-1}$ \\
Masse volumique $\rho$ & 2700 & $kg  \ m^{-3}$ \\
Capacité calorifique $C$ & 897 & $J kg^{-1} K^{-1}$\\
Coefficient de convection $h$ & 10 ou 200 & $W m^{-2} K^{-1}$ \\ 
...
\left( { - \frac{{k_{th}^{}}}{{{{\left( {\Delta x} \right)}^2}}}} \right)T_{p - 1}^{n + 1} + \left( {\frac{{2k_{th}^{}}}{{{{\left( {\Delta x} \right)}^2}}} + \frac{{\rho \;c}}{{\Delta t}} + \frac{{2h}}{e}} \right)T_p^{n + 1} + \left( { - \frac{{k_{th}^{}}}{{{{\left( {\Delta x} \right)}^2}}}} \right)T_{p + 1}^{n + 1} = \frac{{\rho \;c}}{{\Delta t}}T_p^n  + \frac{{2h}}{e}{T_a}
\end{equation}
...
Télécharger le projet source se trouvant sur le serveur.\\
%\tt{/users/commun/respg/phelma\_2/06\_mdf-implicite2011-2012/ailette\_etudiants} \rm \\

\begin{enumerate}
\item Compléter la fonction \tt{implicite.m} \rm
...
\end{equation}
avec 
$r = \sqrt {\frac{{2h}}{{e\,{k_{th}}}}} $. Dans quelle limite peut-on l'obtenir avec le schéma numérique?
\end{enumerate}
\end{document}
