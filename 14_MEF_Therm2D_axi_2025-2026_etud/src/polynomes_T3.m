...
% gauss.x(npi) : abscisses des points de Gauss
% gauss.y(npi) : ordonnees des points de Gauss
...
for npi=1:NPI
    ...
    for ie=1:NBN
% Ajout        
        xg(npi)=xg(npi)+alpha(ie, npi)*xp(ie);
        yg(npi)=yg(npi)+alpha(ie, npi)*yp(ie);
    ...
    J=[dx_du dy_du;     % transposee de la jacobienne
       dx_dv dy_dv];
    detJ(npi)=det(J);    
    ...
    for ie=1:NBN 
        ...
        gradai=invJ*[dalphai_du; dalphai_dv];
