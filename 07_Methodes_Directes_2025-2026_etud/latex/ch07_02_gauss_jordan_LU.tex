...
\lhead{} 
\chead{} 
\rhead{\bfseries Algèbre Linéaire en Matlab} 
...
\title{Algèbre Linéaire en Matlab}
...
Elle consiste à transformer la matrice associée $A=(a_{ij})_{\; 1\leq i \leq n,\;  1\leq j \leq n }$ en une matrice unité en effectuant des combinaisons linéaires de lignes.
...
L'algorithme est itératif et s'effectue en $n$ étapes. A l'étape $k \in [1, n]$, on
...
Divisons la première ligne par le pivot $a_{1,1}$ :
$l_1 \leftarrow {1 \over 2} \times l_1$
...
\right)
\begin{array}{c}
   \\ l_2 \leftarrow l_2 -3 \times l_1   \\ l_3 \leftarrow l_3 -4 \times l_1  \\ 
\end{array}
\end{equation}

Divisons la deuxième ligne par le pivot $a_{2, 2}$ :
$l_2 \leftarrow {2 \over 3} \times l_2$
...
\right)
\begin{array}{c}
l_1 \leftarrow  l_1 -{1 \over 2}  \times l_2   \\     \\ l_3 \leftarrow  l_3 -3 \times l_2  \\ 
\end{array}
\end{equation}

Divisons la troisième ligne par le pivot $a_{3,3}$ : $l_3 \leftarrow {1 \over 4} \times l_3$
...
\right)
\begin{array}{c}
l_1 \leftarrow  l_1 +{7 \over 3}  \times l_3   \\   l_2 \leftarrow  l_2 - {2 \over 3} \times l_3   \\  \\ 
\end{array}
\end{equation}
...
\subsection{Implémentation de Gauss-Jordan en Matlab (niveau *)}

\begin{enumerate} 
 \item Proposez une implémentation d'une fonction {\tt gauss\_jordan} en Matlab
 de l'algorithme précédent, permettant de résoudre un système d'équations linéaires.
 
 \item Traitez quelques cas tests,  vérifier vos résultats.
...
\right)$ 
 ...
 fonction Matlab \\ {\tt eig} pour calculer les valeurs propres.
 \item Pour vérifier l'inégalité \eqref{conderr},  on calculera les normes avec la fonction
 {\tt  norm} en choisissant une norme infinie, par exemple.
 \end{enumerate}
 
\section{Décomposition LU d'une matrice sans pivot (niveau **)}

Le but est ici de décomposer une matrice sous la forme : $A=L\; U$
où 
...
La première étape d'élimination consiste à faire les combinaisons linéaires suivantes: 
 ...
 \begin{array}{c}
 l_1 \leftarrow l_1 \\
   l_2 \leftarrow l_2 +3/10 \times l_1   \\ l_3 \leftarrow l_3 -1/2 \times l_1  \\ 
\end{array}
\end{equation}
...
On fournit l'algorithme suivant :
...
\end{algorithmic}
  ...
  \begin{enumerate} 
 \item Proposez une implémentation en Matlab.
 \item Testez-la en utilisant les matrices fournies précédemment.
 \item Que se passe-t-il quand le pivot est trop petit voire nul?
  \end{enumerate} 
...
La seconde étape de permutation revient à échanger les lignes $2$ et $3$:
...
\begin{equation}
 \begin{array}{c}
  l_3 \leftarrow l_3 +\frac{1}{25} \times l_2 
\end{array}
\end{equation}
On obtient:
\begin{equation}
 U=A^{(2)}=L^{-1}_2 \;  P_2 \; A^{(1)}=
  \left(
\begin{array}{c c c}
1  & 0   &  0  \\
0  &  1   & 0  \\
...
\noindent D'où : 
\begin{equation}
 U = L^{-1}_2 \;  P_2 \;  L^{-1}_1 \;  P_1 \;  A \equiv (PL)^{-1} \; A
 \end{equation}
 
\noindent On en déduit :
\begin{equation}
PL = \Pi_{k=1}^{n-1}  (P_k^t \; L_k) = \Pi_{k=1}^{n-1}  (P_k \; L_k)
\end{equation}
 en remarquant que chaque matrice $P_k$ est symétrique. On construit finalement la matrice globale de permutation $P= \Pi_{k=1}^{n-1}  P_k$
qui n'est pas en général symétrique.
...
Le calcul des matrices $L_k$ s'effectue facilement grâce à la propriété suivante:
 ...
 La matrice triangulaire inférieure $L$ est finalement reconstruite sachant que $P^{-1}=P^t$ :
  \begin{equation}
 L= P^t\;  \Pi_{k=1}^{n-1}  (P_k^t \; L_k) = P^t\;  \Pi_{k=1}^{n-1}  (P_k \; L_k)
 \end{equation}
...
\end{algorithmic}
 ...
 \item Proposez une implémentation en Matlab.
 \item Testez-la en utilisant les matrices fournies précédemment.
 \item Proposez un exemple qui mettait en défaut la simple factorisation LU.
  \end{enumerate} 
